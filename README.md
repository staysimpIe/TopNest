...
项目当前提供 Codex、Gemini、Claude & GPT 三个额度组件，音乐控制、歌词两个音乐组件，以及系统状态组件。整体结构按照通用桌面组件容器设计，后续可以继续扩展天气和快捷工具等组件。
...
- 每秒读取 Windows CPU、物理内存和网络累计计数。
- 展示 CPU、内存占用率，以及实时下载和上传速度。
- 首次采样先展示内存信息，下一次采样后补充 CPU 与网速。
...
| 系统状态 | Windows 系统接口 | CPU、内存占用率及实时上下行速度 | Windows 10 或 Windows 11 |
